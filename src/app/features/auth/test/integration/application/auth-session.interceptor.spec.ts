...
describe('authSessionInterceptor', () => {
  ...
  it('clears the session and redirects when a protected API request receives 401', () => {
    ...
    httpTestingController.expectOne('/api/categories').flush({ message: 'No autorizado' }, { status: 401, statusText: 'Unauthorized' });
  ...
  it('does not redirect when login itself is rejected', () => {
    ...
    httpTestingController.expectOne('/api/auth/login').flush({ message: 'Credenciales inválidas' }, { status: 401, statusText: 'Unauthorized' });
  ...
  it('does not alter failed requests to external resources', () => {
    ...
    httpTestingController.expectOne(externalUrl).flush({ message: 'Forbidden' }, { status: 401, statusText: 'Unauthorized' });
